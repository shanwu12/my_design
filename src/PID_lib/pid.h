...
{
public:
    PID(float _Kp = 2.0, float _Ki = 0.001, float _Kd = 1.0,
        float _integralMin = -200, float _integralMax = 200,
        float _ctrlMin = -10, float _ctrlMax = 10,
        bool _is_debug = false);
    float update(const float currentPoint,
                 const float dt,
                 float *d_error, float *d_pval, float *d_ival, float *d_dval);
    float update(const float currentPoint, const float dt);
